...
for i in range(45,46):
	file_w = open('/Users/anandpopat/desktop/demonetization/demonetization'+str(i)+'_pfc.json','a')###change topic
	file_r = open('/Users/anandpopat/desktop/demonetization/demonetization'+str(i)+'_pft.json','r')###change topic 
	...
	for line in file_r:
		...
		d = json.loads(line)
		if(d["geo"] != None):
			co_list = str(d["geo"]["coordinates"])
			print(co_list)
			co_list = re.sub('\[','',co_list)
			co_list = re.sub('\]','',co_list)
			print(co_list)
			d["anand_coordinates"] = co_list
